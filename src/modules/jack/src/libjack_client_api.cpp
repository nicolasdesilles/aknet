...
Result LibJackClientAPI::activate() {
    ...
    // Note: We don't set a process callback here.
    // Phase 6 will add:
    // - jack_set_process_callback(client_, process_callback, user_data)
    // - Real-time audio processing in the callback
    //
    // For now, the client activates but idles (no audio processing).
